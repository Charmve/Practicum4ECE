...
int dec_init()
{
	tDemo.dec = MppDecoderCreate(DECODE_TYPE_H264);
	if(!tDemo.dec) {
...
void *Encoder_Task(void *)
{
    ...
    printf("Enter Encoder-Decoder  task\n");
    ...
    tDemo.dec->ops->enqueue(tDemo.dec,packet->data,packet->size);
...
void *Decoder_Task(void*)
{
    ...
    printf("Enter Decoder-display task\n");
    while(1){
        frame = tDemo.dec->ops->dequeue(tDemo.dec);
		if(frame != 0) {
            // transfor using rga
            rga->ops->initCtx(rga);
            rga->ops->setSrcBufferPtr(tDemo.rga, frame->data);
            rga->ops->setDstBufferPtr(tDemo.rga, tDemo.image.data);

            rga->ops->setSrcFormat(tDemo.rga, tDemo.enc_format, tDemo.enc_width, tDemo.enc_height);
            rga->ops->setDstFormat(tDemo.rga, tDemo.disp_format, tDemo.disp_width, tDemo.disp_height);
...
int main(int argc, char *argv[])
{
    ...
    tDemo.runflag = 1;
	ret = pthread_create(&pid0, NULL, Capture_Task, NULL);
	ret = pthread_create(&pid1, NULL, Encoder_Task, NULL);
	ret = pthread_create(&pid2, NULL, Decoder_Task, NULL);
