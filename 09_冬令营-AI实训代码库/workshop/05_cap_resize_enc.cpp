...
int enc_frameque_init()
{
    ...
    RgaBuffer *buf;
	for(i = 0; i < BUFFER_COUNT; i++) {
		buf = tDemo.rga->ops->allocDmaBuffer(tDemo.rga,tDemo.enc_format,tDemo.enc_width,tDemo.enc_height);
        tDemo.enc->ops->importBufferFd(tDemo.enc,buf->fd,i);
		printf("Drm alloc: %d,%d, %p, %lu %lu\n", i, buf->fd,buf->ptr, buf->size, tDemo.enc_size);
        tDemo.Enc_buf[i] = buf;
	}
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    tDemo.cap_width  = 1280;
    tDemo.cap_height = 960;
    ...
    tDemo.enc_width = 720;
    tDemo.enc_height= 480;
    tDemo.enc_format= tDemo.cap_format;
    tDemo.enc_size  = 720*480*2;
